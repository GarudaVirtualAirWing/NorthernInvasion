...
ctld.pickupZones = {
    { "mklogizone", "green", -1, "yes", 0 },
    { "kraspashlogizone", "green", -1, "yes", 0 },
    { "krascenterlogizone", "green", -1, "yes", 0 },
    { "krymsklogizone", "green", -1, "yes", 0 },
    { "novologizone", "green", -1, "yes", 0 },
...
ctld.unitActions = {
    ...
    ["Yak-52"] = {crates=true, troops=false},
...
ctld.loadableGroups = {
    ...
    {name = "Tiny Anti Air Group", inf = 1, aa = 2  },
...
ctld.spawnableCrates = {
    ...
    ["Ground Forces"] = {
        ...
        { weight = 1776, desc = "M1A1 Abrams", unit = "M-1 Abrams", side = 2, cratesRequired = 2 },
        ...
        { weight = 700, desc = "M-978 Tanker", unit = "M978 HEMTT Tanker", side = 2, cratesRequired = 1 },
    },
    ["AA Crates"] = {
        -- HAWK System
        ...
        { weight = 1005, desc = "M163 Vulcan", unit = "Vulcan", side = 2, cratesRequired = 1 },
...
function ctld.spawnFOB(_country, _unitId, _point, _name)
  --Spawns the FOB buildings at a given point, returning the FOB object so that 
  --we can spawn crates and troops at the FOB location later.
  --
  --Inputs
  --  _country : int
  --    Country ID of the helicopter deploying the FOB
  --  _unitId : int
  --    Unique ctld unit ID
  --  _point : table
  --    3D location of the FOB
  --  _name : str
  --    Generated name for the FOB
  --
  --Outputs
  --  FOB_Object : obj
  --    A reference to the spawned FOB object ("outpost" in the normal case)

  --Spawn Tire
  ------------------------------------------------------------------------------
  --This will be used as the FOB object since we need to return a static object
  --later.
  local _FOB_Static = {
    ["category"] = "Statics",
    ["type"] = "Black_Tyre",
  ...
  local FOB_Name = mist.dynAddStatic(_FOB_Static).name
  local FOB_Object = StaticObject.getByName(FOB_Name)

  --Spawn Outpost Group
  ------------------------------------------------------------------------------
  --This is used if you want to spawn a watchtower w/o a mission editor object
  local _FOB_Group = {
    ["visible"] = false,
    -- ["groupId"] = _groupId,
    ["hidden"] = false,
    ...
    ["name"] = "FOB Group #" .. _unitId,
    ["task"] = {},
    --added two fields below for MIST
    ["category"] = Group.Category.GROUND,
    ["country"] = _country
  }
  --_spawnedFOB = mist.dynAdd(_FOB_Group)
  _spawnedFOB = Group.getByName(mist.dynAdd(_FOB_Group).name)
  trigger.action.setGroupAIOff(_spawnedFOB)    --Turn off AI


  --Spawn Outpost Group
  ------------------------------------------------------------------------------
  --This is used if you want to spawn a watchtower and outpost w/ mission editor 
  --static objects. Note: This will require clients to log out and log back in 
  --to be able to see the outpost!
  --It needs static objects with the correpsonding names to be placed in the ME
  --in order to function.
  --[[
  local _outpostTable = {
    groupName = "outpost",                     --static object in mission editor
    point = {
      x = _point.x, 
      y = _point.z
    }, 
  ...
  local outpostName = mist.teleportToPoint(_outpostTable).name
  local FOB_Object = StaticObject.getByName(outpostName)

  --Debug
  --trigger.action.outTextForCoalition(2, "FOB Name: "..outpostName, 30)

  local _watchtowerTable = {
    groupName = "watchtower",                  --static object in mission editor
    point = {
      x = _point.x + 14.85714286, 
      y = _point.z - 36.57142857
    }, 
  ...
  mist.teleportToPoint(_watchtowerTable)
  ]]
...
function ctld.spawnCrate(_arguments)
    ...
    local _status, _err = pcall(function(_args)
        ...
        if _crateType ~= nil and _heli ~= nil and ctld.inAir(_heli) == false then
            ...
            if ctld.inPickupZone(_heli).inZone == false then
    ...
    local _status, _result = pcall(function()
        ...
        for _, _name in ipairs(ctld.transportPilotNames) do
            ...
            if _transUnit ~= nil and ctld.inTransitSlingLoadCrates[_name] == nil and ctld.inAir(_transUnit) and ctld.unitCanCarryVehicles(_transUnit) == false then
                ...
                if ctld.unitIsHoverExempt(_transUnit) then
                    break
                end
...
if (not _status) then
    env.error(string.format("CTLD ERROR: %s", _err))
                ...
                function ctld.unpackFOBCrates(_crates, _heli)
                    ...
                    local _details = { point = _centroid, smoke = trigger.smokeColor.Green}
            ...
            function ctld.dropSlingCrate(_args)
                ...
                if _currentCrate == nil then
                    if ctld.hoverPickup and (not ctld.unitIsHoverExempt(_heli)) then
            ...
            function ctld.createRadioBeacon(_point, _coalition, _country, _name, _batteryTime, _isFOB)
                ...
                local _uhfPoint = mist.utils.deepCopy(_point)
                _uhfPoint.x = _uhfPoint.x + 5
              
                local _fmPoint = mist.utils.deepCopy(_point)
                _fmPoint.x = _fmPoint.x - 5
              
                local _uhfGroup = ctld.spawnRadioBeaconUnit(_uhfPoint, _country, "UHF")
                local _vhfGroup = ctld.spawnRadioBeaconUnit(_point, _country, "VHF")
                local _fmGroup = ctld.spawnRadioBeaconUnit(_fmPoint, _country, "FM")
    ...
    function ctld.unitIsHoverExempt(_unit)

        local _type = string.lower(_unit:getTypeName())
      
        for _, _name in ipairs(ctld.hoverExempt) do
            local _nameLower = string.lower(_name)
                    ...
                    if _groupId then
                        ...
                        if ctld.addedTo[tostring(_groupId)] == nil then
                            ...
                            if (ctld.enabledFOBBuilding or ctld.enableCrates) and _unitActions.crates then
                                ...
                                if (ctld.hoverPickup == false) or ctld.unitIsHoverExempt(_unit) then
                                    if  ctld.slingLoad == false then
    ...
    function ctld.notifyGroup(_message, _displayFor, _grp_id)
        trigger.action.outTextForGroup(_grp_id, _message, _displayFor)
        trigger.action.outSoundForGroup(_grp_id, "radiobeep.ogg")
    end
    ...
    function ctld.getJTACStatus(_args)
        ...
        ctld.notifyGroup(_message, 10, _playerUnit:getGroup():getID())
    end
